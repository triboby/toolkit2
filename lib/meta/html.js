...
var URLHANDLER = {
    ...
    sp:(function(){
        var fMap = {
            ...
            position:function(uri,info,ratio){
                var arr = ratio.split(',');
                return util.format(
                    '-%spx -%spx',
                    Math.round(info.x*parseFloat(arr[0])),
                    Math.round(info.y*parseFloat(arr[1]))
                );
            },
            defoult:function(uri,info,ratio){
                var arr = ratio.split(',');
                return util.format(
                    'url(%s) -%spx -%spx',uri,
                    Math.round(info.x*parseFloat(arr[0])),
        ...
        return function(uri,config){
            ...
            return (fMap[arr[1]]||fMap.defoult)(uri,info,arr[2]);
        }
    })(),
    // style resource
...
pro._parseContent = function(file,content,config){
    ...
    config = _util.merge(config,{
        exLinkAttributeFlag:config.
            exLinkAttributeFlag||this.isTemplate
    });
    // parse html
...
pro._embedResource = function(config){
    ...
    // console.log('%j，%j',resConfig, config);
    if (resConfig.core){
        ret.push('core');
    }
 ...
 * @param  {Number} config.mdlVersion  - nej module version mode
...
pro.output = function(config){
    ...
    // merge nej module version mode
    content = content.replace(
        _util.MODULE_VERSION_HOLDER,
        config.mdlVersion
    );
    // cache content
