 ...
 * @param  {Object} options - resource check options
 * @param  {Boolean}options.lnkCheck    - check resource in script
 * @param  {Object} options.lnkOption   - resource check config options
 * @param  {String} options.webRoot     - webapp root
 * @param  {String} options.resRoot     - resource root
 * @param  {Boolean}options.versionStac - add version to resource
 * @param  {String} options.versionStacMode - static version mode
...
pro._checkResInScript = (function(){
    ...
    return function(file,content,options){
        ...
        // check content
        var _this = this,
            lopt = options.lnkOption||{};
        reg.lastIndex = -1;
        return (content||'').replace(
            reg,function($1,$2,$3){
                // check resource uri
                var arr = $3.split('?');
                arr[0] = arr[0].replace('/',options.webRoot);
                var isdir = _fs.isdir(arr[0]);
                // for static resource
                if ((lopt.checkDIR||!isdir)&&
                    arr[0].indexOf(options.resRoot)===0){
                    // check resource exist
                    if (_fs.exist(arr[0])){
                        if (!isdir&&options.versionStac){
                            if (!!arr[1]){
                                _this.emit('warn',{
                                    data:[$3],
                                ...
                                // check resource version with mode
                                arr = _io.adjustResource(
                                    arr[0],options,function(src,dst){
                                        _this.emit('debug',{
                                            data:[src,dst],
                        ...
                        arr[0] = arr[0].replace(options.webRoot,domain);
                        _this.emit('debug',{
                            data:[$3,file,arr[0]],
...
pro.merge = function(config){
    ...
    _sm.concat('core.js', config.coreScript);
    // merge resource
    var map = {
        'core.js':['core.js']
    };
    ...
    // console.log('%j',Object.keys(map));
    this._scripts = map;
};
/**
 * minify script
 * @param  {Object} config - config object
 * @param  {String} config.bags        - name bags file path
 * @param  {String} config.level       - obfuscate level
 * @param  {String} config.sourcemap   - output source map
 * @param  {Object} config.variables   - global variables
 * @param  {String} config.compatible  - use nej compatible mode
 * @param  {String} config.dropconsole - drop console code
...
pro.output = function(config){
    ...
    this._list.forEach(function(parser){
        ...
        tpl.push(parser.isTemplate);
    });
    // update nej module version
    var mod = config.mdlVersion,
        exf = config.outHtmlRoot!==config.outTmplRoot
            ? config.outTmplRoot : '';
    // console.log('%j',ret);
    ret.forEach(function(file,index){
        ...
        _fs.mkdir(path.dirname(file));
        _io.output(file,content,config.charset);
        // output nej module file with path version
        if (mod===1&&!tpl[index]){
            file = file.replace(
                /(\.[^.\/]*?)$/,
                '_'+_util.md5(content)+'$1'
            );
            this.emit('debug',{
                data:[file],
                message:'output file %s'
